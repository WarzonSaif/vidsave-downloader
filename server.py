    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for f in info.get('formats', []):
                ...
                # For Pinterest and Reddit, accept any format with video
                if platform in ['pinterest', 'reddit']:
                    if vcodec and vcodec != 'none' and f.get('url'):
                ...
                # For other platforms, only formats with BOTH video AND audio
                elif vcodec and vcodec != 'none' and acodec and acodec != 'none':
                    # Build proper quality label
                    quality_label = f.get('quality_label')
                    if not quality_label and height:
                        quality_label = f'{height}p'
                    elif not quality_label:
            ...
            # Get unique qualities (prefer formats with audio)
            seen = set()
            unique_formats = []
            for f in formats:
...
def download_video():
    ...
    try:
        ...
        print(f"Getting direct URL for: {url} | format: {format_id}")
        
        # Detect platform
        platform = get_platform(url)
        
        # Build format spec - NO ffmpeg merging
        ...
        if platform == 'reddit':
            ...
            # Pinterest needs special handling - use best format
            ydl_opts['format'] = 'bestvideo+bestaudio/best'
            ydl_opts['merge_output_format'] = 'mp4'
        # Dailymotion removed
        ...
        print(f"Direct URL found for: {title}")
        
        # Return the direct URL to frontend
        # Frontend will open it directly - instant download!
