        ...
        row.innerHTML = `
            <div class="dl-left">
                ...
                <div class="dl-info">
                    ...
                    <div class="dl-meta">${fmt.ext ? fmt.ext.toUpperCase() : 'MP4'}${size ? ' · ' + size : ''}</div>
                </div>
            </div>
            <button class="dl-btn" onclick="startDownload(this, '${encodeURIComponent(url)}', '${fmt.format_id}')">
                <i class="fas fa-download"></i> Download
            </button>
        `;
...
function downloadSelected() {
    ...
    startDownload(btn, encodeURIComponent(url), fmt.format_id);
}

// ─── DOWNLOAD ───────────────────────────────────────────────────────────────
async function startDownload(btn, encodedUrl, formatId) {
    ...
    try {
        ...
        // Server returns direct URL - open it instantly!
        if (data.direct_url) {
